...
import { requestStudySuggestion, type GuidedSuggestion } from './studyApi'
import {
...
export function PreparationWorkspace({ studies, today = new Date() }: PreparationWorkspaceProps) {
  ...
  const [includePersonalAnswer, setIncludePersonalAnswer] = useState(false)
  ...
  async function generateSuggestion() {
    ...
    setGenerationError('')
    try {
      const result = await requestStudySuggestion({
        source: { canonicalUrl: activeStudy.sourceUrl },
        questionId: `${activeStudy.id}-theme`,
        questionText: activeStudy.theme,
        paragraphs: [
          ...activeStudy.days.map((day) => `${day.day}: ${day.focus}`),
          `${activeStudy.memoryVerse.reference}: ${activeStudy.memoryVerse.application}`,
        ],
        ...(includePersonalAnswer && activeDraft.personalAnswer.trim() !== ''
          ? { personalAnswer: activeDraft.personalAnswer }
          : {}),
        mode: 'guided',
      })
      ...
      setGenerationError(error instanceof Error ? error.message : 'La génération a échoué.')
    }
  }
    ...
    <section className="assistant-workspace" id="assistant" aria-labelledby="assistant-title">
      ...
      <div className="assistant-layout">
        ...
        <div className="assistant-editor">
          <ModeChoice mode={activeDraft.mode} onChange={(mode) => { setSuggestion(null); setGenerationState('idle'); setGenerationError(''); updateDraft({ mode }) }} />
          ...
          {activeDraft.mode === 'guided' && <section className="assistant-ai" aria-live="polite"><h2>Suggestions IA</h2><p>La suggestion est générée à partir de la source officielle. Votre réponse personnelle reste locale, sauf si vous autorisez explicitement son envoi.</p><label><input type="checkbox" checked={includePersonalAnswer} onChange={(event) => setIncludePersonalAnswer(event.target.checked)} /> Inclure ma réponse personnelle</label><button type="button" onClick={generateSuggestion} disabled={generationState === 'loading'}>{generationState === 'loading' ? 'Génération en cours…' : 'Générer une suggestion IA'}</button>{generationState === 'error' && <p role="alert">{generationError}</p>}{suggestion?.answers[0] && <article><h3>Suggestion</h3><p>{suggestion.answers[0].suggestedAnswer}</p><p><strong>Paragraphes de soutien :</strong> {suggestion.answers[0].supportingParagraphs.join(' · ')}</p>{suggestion.warnings.map((warning) => <p key={warning} role="note">{warning}</p>)}</article>}</section>}
